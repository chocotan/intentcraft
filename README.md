...
每个 skill 的 `SKILL.md` 都声明输入、边界、完成标准和产出。需要保存、跨会话或用户明确要求时，持久文档只写对应 `docs/ic-*/`，文件名使用 `YYYY-MM-DD-<topic>.md`。

同一主题同一阶段继续推进时更新原文件；新的研究快照、Design/Plan 阶段、代码批次或审查阶段才新建日期文件。文件内记录 `created`、`updated` 和 `status`，不使用 `final-v2-final` 之类名称。
